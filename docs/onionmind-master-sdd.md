...
**Data:** 2026-08-22
**Status:** Fase 1 concluída e validada (2026-08-22, contra a rede Tor real). Fase 2 (busca semântica) mergeada nos PRs #5 e #6 — código completo, gate de saída ainda não re-executado contra Ollama + Qdrant ao vivo. Fase 3 (IA generativa + Cost Optimizer, branch `feat/phase-3-llm-integration`) com o código completo: `AIOrchestrator` real multi-provider, processors de enriquecimento, filtro de conteúdo ilegal, métricas Micrometer, `Fase3AiGenerationGateTest` verde com fakes — validação end-to-end contra Groq/Gemini reais pendente (ver `onionmind-fase3-sdd.md` §11.4). Fase 4 (Knowledge Graph + Versionamento + alertas, branch `feature/phase4-knowledge-graph`, change OpenSpec `phase4-knowledge-graph`) com o código completo: Event Publication Registry provisionado (fecha a nota P2-5), `PageIndexedEvent`/`PageContentLookup` (ADR-009), módulos `graph` (extração de entidades + Neo4j) e `intelligence` (diff de versão + alertas por regra), `Fase4KnowledgeGraphGateTest` verde com fakes de `AIOrchestrator`/`GraphStore` — validação end-to-end contra Neo4j/LLM reais pendente (ver `onionmind-fase4-sdd.md` §Validação real).
